      ...
      <!--breadcrumb-->
                ...
                <div class="container">
                    ...
                    <form action="{{ route('blogCategories.update', $category->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <input class="form-control" type="text" name="category_name" value="{{ old('category_name', $category->category_name) }}" required>
                        <button type="submit" class="btn mt-4 btn-primary">Update Category</button>
                    </form>
